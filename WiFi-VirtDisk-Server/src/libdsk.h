...
typedef enum
{
	...
	FMT_C64_35,	/* Commodore C64 35 tracks */
	FMT_C64_40,	/* Commodore C64 40 tracks */
	FMT_C64_CPM,	/* Commodore C64 CP/M */
	FMT_UNKNOWN = -1
} dsk_format_t;
...
/* Recording modes 0x10-0x2F are Apple GCR, defined as the GCR format byte 
 ...
 * These fall into the range RECMODE_GCR_FIRST - RECMODE_GCR_LAST */
	...
	RECMODE_GCR_C64    = 0x0030,	  /* Commodore 64 GCR */
...
/* As for "open", but performs a diagnostic dump of the disk image to the
 * output callback */
LDPUBLIC32 dsk_err_t  LDPUBLIC16 dsk_diagopen(DSK_PDRIVER *self, 
			const char *filename, 
			const char *type,
			const char *compress,
