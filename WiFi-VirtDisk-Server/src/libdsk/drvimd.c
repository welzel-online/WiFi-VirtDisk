...
static dsk_err_t imd_load_track(IMD_DSK_DRIVER *self, dsk_ltrack_t count, 
	FILE *fp, DSK_REPORTFUNC diagfunc)
{
	...
	{
		tmp.imdt_seclen = dsk_expand_psh(psh);
		for (n = 0; n < 256; n++) datalen[n] = tmp.imdt_seclen;
	}
	diagnl(diagfunc);
	diaghead(diagfunc, "Header for track %ld", (long)count);
	diaghex (diagfunc, fpos, &tmp.imdt_mode, 1, "Recording mode");
	diaghex (diagfunc, fpos + 1, &tmp.imdt_cylinder, 1, "Cylinder = %d",
			tmp.imdt_cylinder);
	diaghex (diagfunc, fpos + 2, &tmp.imdt_head, 1, "Head = %d",
			tmp.imdt_head);
	diaghex (diagfunc, fpos + 3, &tmp.imdt_sectors, 1, "Sectors = %d",
			tmp.imdt_sectors);
	diaghex (diagfunc, fpos + 4, &ch, 1, "Sector size = %d",
			tmp.imdt_seclen);
...
/*	printf("Mode %d Cyl %d Head %d Secs %d Size %d\n",
	...
	/* Load sector IDs */	
	diagrawrange(diagfunc, fp, fpos + 5, tmp.imdt_sectors, "Sector IDs");
	for (n = 0; n < tmp.imdt_sectors; n++)
	{
		c = fgetc(fp); 	
		if (c == EOF) 
	...
	{
		fpos = ftell(fp);
		diagrawrange(diagfunc, fp, fpos, tmp.imdt_sectors, "Sector cylinder map");
		for (n = 0; n < tmp.imdt_sectors; n++)
		{
			c = fgetc(fp); 	
			if (c == EOF) 
	...
	{
		fpos = ftell(fp);
		diagrawrange(diagfunc, fp, fpos, tmp.imdt_sectors, "Sector head map");
		for (n = 0; n < tmp.imdt_sectors; n++)
		{
			c = fgetc(fp); 	
			if (c == EOF) 
	...
	if (tmp.imdt_seclen == 0xFFFF)
	{
		int l, h;
		fpos = ftell(fp);
		diagrawrange(diagfunc, fp, fpos, tmp.imdt_sectors * 2, "Sector lengths");
	...
	for (n = 0; n < tmp.imdt_sectors; n++)
	{
		...
		ch = c;
		diaghex(diagfunc, fpos, &ch, 1, "Sector %d status", n);
		status = c;
		...
		switch(status)
		{
			...
			case ST_CDELERR: 	
				...
				ch = c;
				diaghex(diagfunc, fpos + 1, &ch, 1, "Sector %d compressed", n);
				break;
			/* Uncompressed */
			case ST_NORMAL: 		
			case ST_DELETED:
			...
			case ST_DELERR: 
				...
				diaghex(diagfunc, fpos + 1, buf, datalen[n], "Sector %d uncompressed", n);
...
dsk_err_t imd_open(DSK_DRIVER *self, const char *filename, DSK_REPORTFUNC diagfunc)
{
	FILE *fp;
...
/*	printf("Header='%s' pos=%ld\n", comment, ftell(fp)); */
	...
	diaghead(diagfunc, "IMD disk image");
	diaghex(diagfunc, 0, (unsigned char *)comment, ccmt, 
	...
	if (termch != 0x1A)
	{
		...
		diaghex(diagfunc, fpos, (unsigned char *)comment, ccmt, 
			"Comment");
		comment[ccmt - 1] = 0;
	...
	while (!feof(fp))
	{
		err = imd_load_track(imdself, count, fp, diagfunc);
