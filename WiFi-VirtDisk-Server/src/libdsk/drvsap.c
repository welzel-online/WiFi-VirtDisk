...
dsk_err_t sap_open(DSK_DRIVER *self, const char *filename, DSK_REPORTFUNC diagfunc)
{
	FILE *fp;
	...
	diaghead(diagfunc, "SAP disk image");
	diaghex(diagfunc, 0, (unsigned char *)header, 1, "Drive type");
	diaghex(diagfunc, 1, (unsigned char *)header + 1, 65, "Magic number");
	fpos = 66;
	...
	for (nt = 0; nt < tracks; nt++)
	{
		...
		diaghead(diagfunc, "Track %d (Cylinder %d head %d)",
			nt, cylinder, head);
		...
		for (sec = 0; sec < SAP_SECTORS; sec++)
		{
			...
			diaghex(diagfunc, fpos, secbuf, 1, 
					"Sector %d size", sec);
			diaghex(diagfunc, fpos + 1, secbuf + 1, 1, 
					"Sector %d protection", sec);
			diaghex(diagfunc, fpos + 2, secbuf + 2, 1, 
					"Sector %d cylinder ID", sec);
			diaghex(diagfunc, fpos + 3, secbuf + 3, 1, 
					"Sector %d sector ID", sec);
			fpos += 4;
			switch(secbuf[0] & 3)
			{
				case 0: secsize = 256; 
			...
			diaghex(diagfunc, fpos, secbuf + 4, secsize, "Sector %d data", sec);
			fpos += secsize;
			...
			diaghex(diagfunc, fpos, secbuf + 4 + secsize, 2, "Sector %d CRC %s", sec, (crc == dcrc) ? "" : "bad");
			fpos += 2;
			if (crc != dcrc)
			{
