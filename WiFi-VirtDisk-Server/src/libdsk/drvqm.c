 ...
 * used by drv_qm_open                          *
 ************************************************/
static dsk_err_t drv_qm_load_header(QM_DSK_DRIVER * qm_self, unsigned char *header, DSK_REPORTFUNC diagfunc)
{
	int i;
	...
	diaghead(diagfunc, "CopyQM disk image");
	diaghex (diagfunc, QM_H_BASE,        header + QM_H_BASE,        3, "Magic number");
	diaghex (diagfunc, QM_H_SECSIZE,     header + QM_H_SECSIZE,     2, "BPB sector size = %d", qm_self->qm_h_bpb_sector_size);
	diaghex (diagfunc, QM_H_SECCLUS,     header + QM_H_SECCLUS,     1, "BPB sectors / cluster = %d", qm_self->qm_h_bpb_secclus);
	diaghex (diagfunc, QM_H_RESERVED,    header + QM_H_RESERVED,    2, "BPB reserved sectors = %d", qm_self->qm_h_bpb_reserved);
	diaghex (diagfunc, QM_H_FATS,        header + QM_H_FATS,        1, "BPB FAT copies = %d", qm_self->qm_h_bpb_fats);
	diaghex (diagfunc, QM_H_ROOTENTRIES, header + QM_H_ROOTENTRIES, 2, "BPB root directory entries = %d", qm_self->qm_h_bpb_rootentries);
	diaghex (diagfunc, QM_H_SECTOTL,     header + QM_H_SECTOTL,     2, "BPB total sectors = %d", qm_self->qm_h_bpb_total_sectors);
	diaghex (diagfunc, QM_H_MEDIA_ID,    header + QM_H_MEDIA_ID,    1, "BPB media ID = 0x%02x", qm_self->qm_h_bpb_media_id);
	diaghex (diagfunc, QM_H_SECFAT,      header + QM_H_SECFAT,      2, "BPB sectors per FAT = %d", qm_self->qm_h_bpb_secfat);
	diaghex (diagfunc, QM_H_SECPTRK,     header + QM_H_SECPTRK,     2, "BPB sectors per track = %d", qm_self->qm_h_bpb_sectrack);
	diaghex (diagfunc, QM_H_HEADS,       header + QM_H_HEADS,       2, "BPB heads = %d", qm_self->qm_h_bpb_heads);
	diaghex (diagfunc, QM_H_HIDDEN,      header + QM_H_HIDDEN,      4, "BPB hidden sectors = %d", qm_self->qm_h_bpb_hidden);
	diaghex (diagfunc, 0x18,             header + 0x18,             4, "BPB large total sectors");
	diaghex (diagfunc, QM_H_DESCR,       header + QM_H_DESCR, QM_H_DSCR_SIZE,"Disk description");
	diaghex (diagfunc, QM_H_BLIND,       header + QM_H_BLIND,       1,"Blind copy");
	diaghex (diagfunc, QM_H_DENS,        header + QM_H_DENS,        1,"Density");
	diaghex (diagfunc, QM_H_USED_CYL,    header + QM_H_USED_CYL,    1,"Cylinders used");
	diaghex (diagfunc, QM_H_TOTL_CYL,    header + QM_H_TOTL_CYL,    1,"Cylinders total");
	diaghex (diagfunc, QM_H_DATA_CRC,    header + QM_H_DATA_CRC,    4,"Data CRC");
	diaghex (diagfunc, QM_H_LABEL,       header + QM_H_LABEL, QM_H_LBL_SIZE ,"Label");
	diaghex (diagfunc, QM_H_TIME,        header + QM_H_TIME,        2, "Time");
	diaghex (diagfunc, QM_H_DATE,        header + QM_H_DATE,        2, "Date");
	diaghex (diagfunc, QM_H_CMT_SIZE,    header + QM_H_CMT_SIZE,    2, "Comment size");
	diaghex (diagfunc, QM_H_SECBASE,     header + QM_H_SECBASE,     1, "First sector number");
	diaghex (diagfunc, 0x72,             header + 0x72,             2, "Unused?");
	diaghex (diagfunc, QM_H_INTLV,       header + QM_H_INTLV,       1, "Interleave");
	diaghex (diagfunc, QM_H_SKEW,        header + QM_H_SKEW,        1, "Skew");
	diaghex (diagfunc, QM_H_DRIVE,       header + QM_H_DRIVE,       1, "Drive");
	diaghex (diagfunc, 0x77,             header + 0x77,            13, "Unknown");
	diaghex (diagfunc, QM_H_HEAD_CRC,    header + QM_H_HEAD_CRC,    1, "Header CRC");
	...
	DSK_REPORTFUNC diagfunc)
{
	...
	if (rs->len == 0)
	{
		...
		if (fread(buf, 1, 2, rs->fp) < 2) 
		{
#ifdef DRV_QM_DEBUG
		fprintf(stderr, "qm: Unexpected EOF at position 0x%08lx\n", fpos);
#endif
		...
		{
			diaghex(diagfunc, fpos, buf, 2, "Repeated run of %d", -rs->len);
			nextc = fgetc(rs->fp);
			if (nextc == EOF) return DSK_ERR_CORRUPT;
			nc = nextc;
			diaghex(diagfunc, fpos + 2, &nc, 1, "Pattern");
		}
		else
		{
			if (diagfunc) 
			{
				diaghex(diagfunc, fpos, buf, 2, "Literal run of %d", rs->len);
				diagrawrange(diagfunc, rs->fp, fpos + 2, rs->len, "Literal data");
			}
...
static dsk_err_t drv_qm_blank_track(QM_DSK_DRIVER *qm_self, dsk_pcyl_t cyl,
		dsk_phead_t head)
{
	LDBS_TRACKHEAD *trkh;
	dsk_psect_t sec;
	size_t seclen = qm_self->qm_h_bpb_sector_size;
	dsk_err_t errcond;

	trkh = ldbs_trackhead_alloc(qm_self->qm_h_bpb_sectrack);
	if (!trkh)
	{
...
/* CopyQM files don't hold GAP3 -- take a wild guess. */
	...
	for (sec = 0; sec < trkh->count; sec++)
	{
		...
		trkh->sector[sec].id_sec  = sec + 1 + qm_self->qm_h_secbase;
	...
	/* All sectors written */
	errcond = ldbs_put_trackhead(qm_self->qm_super.ld_store, trkh,
				cyl, head);
	ldbs_free(trkh);
	return errcond;
}
		...
		DSK_REPORTFUNC diagfunc)
{
	...
	/* [LibDsk 1.5.19] Only read up to qm_h_used_cyls. Higher-numbered
	 * cylinders are assumed blank and not stored in the file */
	for (cyl = 0; cyl < qm_self->qm_h_used_cyls; cyl++)
	{
		...
		DSK_REPORTFUNC diagfunc)
{
	...
	/* Load the header */
	errcond = drv_qm_load_header(qm_self, header, diagfunc);
	if (errcond)
	{
		fclose(fp);
		dsk_free(qm_self->qm_filename);
	...
	if(errcond == DSK_ERR_OK && qm_self->qm_h_comment_len)
	{
		...
		if(comment_buf)
		{
			...
			diaghex(diagfunc, QM_HEADER_SIZE, (unsigned char *)comment_buf, 
					qm_self->qm_h_comment_len, "Comment");
		}
	}
	/* Load the rest */
	if(errcond == DSK_ERR_OK)
	{
		errcond = drv_qm_load_image(qm_self, fp, diagfunc);
		if(errcond != DSK_ERR_OK)
		{
#ifdef DRV_QM_DEBUG
...
/* Cylinders between qm_h_used_cyls and qm_h_total_cyls to be filled 
 * with blanks */
#ifdef DRV_QM_DEBUG
	fprintf(stderr, "Blanking unused cylinders %d to %d",
			qm_self->qm_h_used_cyls, qm_self->qm_h_total_cyls);
#endif
	for (cyl = qm_self->qm_h_used_cyls; cyl < qm_self->qm_h_total_cyls; 
		cyl++)
	{
		for (head = 0; head < qm_self->qm_h_bpb_heads; head++)
		{
			errcond = drv_qm_blank_track(qm_self, cyl, head);	
			if (errcond) 
			{
				ldbs_close(&qm_self->qm_super.ld_store);
				return errcond;
				...
				LDBS_TRACKHEAD *th, void *param)
{
	...
	{
		track_size += dsk_expand_psh(th->sector[n].id_psh);
	}
