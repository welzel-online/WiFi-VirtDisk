				...
				DSK_REPORTFUNC diagfunc)
{
	...
	diaghead(diagfunc, "Sector header: Cylinder %d  Head %d  Sector %d",
			cyl, head, nsec);
	fpos = ftell(self->tele_fp);
	...
	trkh->sector[nsec].datalen = dsk_expand_psh(buf[3]);
	syndrome = buf[4];
	/* buf[5] is the CRC, ignored on load */

	diaghex(diagfunc, fpos + 0, buf + 0, 1, "ID: Cylinder");
	diaghex(diagfunc, fpos + 1, buf + 1, 1, "ID: Head");
	diaghex(diagfunc, fpos + 2, buf + 2, 1, "ID: Sector");
	diaghex(diagfunc, fpos + 3, buf + 3, 1, "ID: Sector size");
	diaghex(diagfunc, fpos + 4, buf + 4, 1, "Syndrome");
	diaghex(diagfunc, fpos + 5, buf + 5, 1, "Header CRC");
	...
	if (err) return err;
	diaghex(diagfunc, fpos + 6, buf + 0, 2, "Sector data length");
	diaghex(diagfunc, fpos + 8, buf + 2, 1, "Sector data encoding");
...
//		ldbs_peek2(buf), encoding, ulen));
	...
	switch(encoding)
	{
		case 0:	/* Uncompressed */
			...
			diaghex(diagfunc, fpos + 9, secbuf, ulen, 
		...
		case 1:	/* One pattern */
			...
			diaghex(diagfunc, fpos + 9, pattern, 2, 
				"RLE1: Repetitions = %d", ldbs_peek2(pattern));
			diaghex(diagfunc, fpos + 11, pattern + 2, 2, 
				"RLE1: Pattern");
		...
		case 2:	/* More than one pattern */
			...
			while (pos < ulen)
			{
				...
				ptype = pattern[0];
				plen = pattern[1];
				diaghex(diagfunc, fpos,  pattern, 1, "RLE2: Type");
				diaghex(diagfunc, fpos+1, pattern+1, 1, "RLE2: Length");
				/* Uncompressed run? */
				if (ptype == 0)
				{
					err = tele_fread(self, pattern, plen);
					if (err) { dsk_free(secbuf); return err; }
					diaghex(diagfunc, fpos+2, pattern, plen, "RLE2: Uncompressed data");
				...
				if (err) { dsk_free(secbuf); return err; }
				diaghex(diagfunc, fpos+2, pattern, 1 << ptype, "RLE2: Compressed data");
				for (n = 0; n < plen; n++)
				{
...
/* Now we're reasonably sure it's Teledisk, do a diagnostic dump if required */
	diaghead(diagfunc, "Teledisk file header");
	diaghex(diagfunc, 0, header + 0, 2, "Magic number");
	diaghex(diagfunc, 2, header + 2, 1, "Volume sequence");	
	diaghex(diagfunc, 3, header + 3, 1, "Volume ID");	
	diaghex(diagfunc, 4, header + 4, 1, "Version");	
	if (header[5] & 0x80)
	{
		diaghex(diagfunc, 5, header + 5, 1, "Data rate  all tracks FM");
		all_fm = 1;
	}
	else
	{
		diaghex(diagfunc, 5, header + 5, 1, "Data rate");	
	}
	diaghex(diagfunc, 6, header + 6, 1, "Drive type");	
	if (header[7] & 0x80)
	{	
		diaghex(diagfunc, 7, header + 7, 1, "Double track (comment present)");	
	}
	else
	{
		diaghex(diagfunc, 7, header + 7, 1, "Double track");	
	}
	diaghex(diagfunc, 8, header + 8, 1, "DOS mode");	
	diaghex(diagfunc, 9, header + 9, 1, "Sides");	
	diaghex(diagfunc,10, header +10, 2, "Header CRC");	
	...
	if (self->tele_head.doubletrack & 0x80)
	{
		...
		diaghead(diagfunc, "Comment record");
		diaghex(diagfunc, fpos, header, 2,  "Comment CRC");
		diaghex(diagfunc, fpos + 2, header + 2, 2,  "Comment length");
		diaghex(diagfunc, fpos + 4, header + 4, 3, 
			"Date: %04d-%02d-%02d", 
			header[4]+1900, 1+(header[5] % 12), header[6] % 100);

		diaghex(diagfunc, fpos + 7, header + 7, 3, 
			"Time: %02d:%02d:%02d", 
			header[7] % 100, header[8] % 100, header[9] % 100);
		...
		diaghex(diagfunc, fpos + 10, (unsigned char *)comment_data, 
				comment_len, "Comment");	
	...
	while (!feof(self->tele_fp))
	{
		...
		diaghead(diagfunc, "Track header");
		diaghex(diagfunc, fpos, header, 1, 
			"Track header: SPT      = %3d",(signed char)(header[0]));
		/* header[0] = sectors / track, if 0xFF then break */
		if (header[0] == 0xFF) break;
		diaghex(diagfunc, fpos + 1, header + 1, 1, 
			"Track header: Cylinder = %3d", header[1]);
		if (all_fm)
		{
			diaghex(diagfunc, fpos + 2, header + 2, 1, 
				"Track header: Head     = %3d", 
				header[2] & 0x7F);
		}
		else
		{
			diaghex(diagfunc, fpos + 2, header + 2, 1, 
				"Track header: Head     = %3d %s", 
				header[2] & 0x7F, (header[2] & 0x80) ? "FM" : "MFM");
		}
		diaghex(diagfunc, fpos + 3, header + 3, 1, 
			"Track header: CRC");
		cyl = header[1];
		head = header[2] & 0x7F;
...
/* TD0 files don't hold GAP3 -- take a wild guess. */
		...
		{
			err = convert_sector(self, trkh, cyl, head, n, diagfunc);
			if (err)
			{
				ldbs_free(trkh);
				ldbs_close(&self->tele_super.ld_store);
