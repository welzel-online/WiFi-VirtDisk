...
static dsk_err_t expand(FILE *fp, unsigned char *trackbuf, size_t tracklen,
			size_t compressed_len, DSK_REPORTFUNC diagfunc)
{
	int c, d;
	size_t offset = 0;
	unsigned char ch;
	...
	while (compressed_len)
	{
		...
		ch = c;
		diaghex(diagfunc, fpos, &ch, 1, "Literal run of %d bytes", ch);
		--compressed_len; 
		if (!compressed_len) 
		{
#ifndef WIN16
		...
		diaghex(diagfunc, fpos + 1, trackbuf + offset, c, 
				"Literal data");
		fpos += (c + 1);
		compressed_len -= c;
		offset += c;
		if (!compressed_len) return DSK_ERR_OK;
		c = fgetc(fp); 	/* Length of compressed data */
		if (c == EOF) 
		{
#ifndef WIN16
		...
		ch = c;
		diaghex(diagfunc, fpos++, &ch, 1, "RLE run of %d bytes", ch);
		--compressed_len; 
		if (!compressed_len) return DSK_ERR_NOTME;
		d = fgetc(fp); 	/* Byte to repeat */
		--compressed_len; 
		ch = d;
		diaghex(diagfunc, fpos++, &ch, 1, "RLE pattern");
		/* Unexpected EOF */
		if (d == EOF) 
		{
#ifndef WIN16
...
dsk_err_t qrst_open(DSK_DRIVER *self, const char *filename, DSK_REPORTFUNC diagfunc)
{
	DSK_GEOMETRY geom;
 ...
 * boot sector be the final arbiter. */
	...
	diaghead(diagfunc, "Quick Release Sector Transfer disk image");
	diaghex (diagfunc, 0, header,     5, "Magic number");
	diaghex (diagfunc, 5, header + 5, 3, "Unknown");
	diaghex (diagfunc, 8, header + 8, 4, "Checksum");
	diaghex (diagfunc,12, header +12, 1, "Disk capacity");
	diaghex (diagfunc,13, header +13, 1, "Volume %d", header[13]);
	diaghex (diagfunc,14, header +14, 1, " of %d", header[14]);
	diaghex (diagfunc,15, header +15,60, "Title");
	diaghex (diagfunc,75, header +75, sizeof(header) - 75, "Description");
	...
	while (fread(chbuf, 1, sizeof(chbuf), fp) == sizeof(chbuf))
	{
		...
		diaghead(diagfunc, "Cylinder %d head %d", cylinder, head);
		diaghex(diagfunc, pos - 3, chbuf, 1, "Cylinder");
		diaghex(diagfunc, pos - 2, chbuf + 1, 1, "Head");
		diaghex(diagfunc, pos - 1, chbuf + 2, 1, "Compression");
		...
		switch (chbuf[2])
		{
			case 0: /* Uncompressed */
				...
				diaghex(diagfunc, pos, trackbuf, tracklen,
						"Uncompressed data");
				break;
			case 1: /* Blank */
				c = fgetc(fp);
				...
				ch = c;
				diaghex(diagfunc, pos, &ch, 1,
						"Fill track with 0x%02x", ch);
			...
			case 2: /* Compressed */
				...
				{
					diaghex(diagfunc, pos, chbuf, 2,
						"Length of compressed data");
					err = expand(fp, trackbuf, tracklen, 
						chbuf[0] + 256 * chbuf[1],
						diagfunc);
				}
				break;
