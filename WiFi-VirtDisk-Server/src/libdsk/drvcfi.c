...
static dsk_err_t cfi_uncompress(unsigned char *cdata, unsigned short clen,
				...
				long fpos, DSK_REPORTFUNC diagfunc)
{
	...
	*ulen = 0;
	...
	while (clen)
	{
		...
		diaghex(diagfunc, fpos + (cdp - cdata), cdp, 2, "Block length = %d %s", blklen & 0x7FFF, (blklen & 0x8000) ? "Compressed" : "Uncompressed");
	...
	/* There must be at least 3 bytes for this block! */
			...
			blklen &= 0x7FFF;
			clen -= 3;
			diaghex(diagfunc, fpos + (cdp - cdata), cdp, 1, "Pattern");
			if (udata)
			{
	...
	/* There must be room for this block in the buffer! */
			...
			clen -= (blklen + 2);
			diaghex(diagfunc, fpos + (cdp - cdata), cdp, blklen, "Uncompressed data");
			if (udata)
			{
...
static dsk_err_t cfi_load_track(CFI_DSK_DRIVER *self, dsk_ltrack_t trk, 
		FILE *fp, DSK_REPORTFUNC diagfunc)
{
	...
	if (diagfunc)
	{
		diaghead(diagfunc, "Track %d", trk);
		clbuf[0] = clen & 0xFF;
		clbuf[1] = clen >> 8;
		diaghex(diagfunc, fpos, clbuf, 2, "Stored length = %d", clen);
	}
	...
	err = cfi_uncompress(cbuf, clen, NULL, &ulen, fpos + 2, NULL);
	if (err)
	{
		dsk_free(cbuf);
		return err;
	...
	/* Decompress track */
	err = cfi_uncompress(cbuf, clen, ubuf, &ulen, fpos + 2, diagfunc);
	if (err)
	{
		dsk_free(ubuf);
		dsk_free(cbuf);
...
dsk_err_t cfi_open(DSK_DRIVER *self, const char *filename, DSK_REPORTFUNC diagfunc)
{
	FILE *fp;
	...
	nt = 0;
	diaghead(diagfunc, "Compressed Floppy Image disk file");
	dsk_report("Loading CFI file into memory");
	while (!feof(fp))
	{
		err = cfi_load_track(cfiself, nt++, fp, diagfunc);
