...
#include "virtDisk.h"                     // WiFi VirtDisk
...
const byte    aBootAddr    = 16;          // Internal EEPROM address for the AUTOBOOT start address
const byte    virtDiskAddr = 18;          // Internal EEPROM address for selecting the WiFi-VirtDisk
...
byte          virtDiskFlag;               // Set to 1 if WiFi-VirtDisk is enabled, otherwise SD disk is used
...
// ------------------------------------------------------------------------------
  ...
  // Read the WiFi-VirtDisk Flag
  virtDiskFlag = EEPROM.read(virtDiskAddr);
  if (virtDiskFlag == 0xFF)
  {
    EEPROM.update(virtDiskAddr, 0);
    virtDiskFlag = 0;
  }
  ...
  if(virtDiskFlag)
  {
    Serial.println(F("IOS: WiFi-VirtDisk Support enabled"));
  }
  // spiTest();
  ...
  // Get the autoboot start address
  autoBootAddr = EEPROM.read(aBootAddr) | ( ((uint16_t)EEPROM.read(aBootAddr+1) << 8) & 0xFF00 );
  ...
  //   or an invalid bootMode code was read from internal EEPROM
  {
    ...
    // WiFi-VirtDisk Support
    Serial.print(F(" A: Toggle WiFi-VirtDisk support (->"));
    if(!virtDiskFlag) Serial.println(F("ON)"));
    else Serial.println(F("OFF)"));
    maxSelChar = 'a';
    ...
    do
    {
      ...
      switch (inChar)
      {
        ...
        case '4':                                   // Change the AUTOBOOT start address
          ...
          if( input( autoBootAddrStr, 6, 2 ) == true )
          {
            ...
            if( tempAddr <= 65535 )
            {
              ...
              EEPROM.update( aBootAddr, (uint8_t)(autoBootAddr % 256) );
              EEPROM.update( aBootAddr + 1, (uint8_t)(autoBootAddr / 256) );
        ...
        case 'a':                                   // Toggle WiFi-VirtDisk support
        case 'A':
          virtDiskFlag = !virtDiskFlag;             // Toggle WiFi-VirtDisk flag
          EEPROM.update(virtDiskAddr, virtDiskFlag); // Save it to the internal EEPROM
        break;
...
{
  if(virtDiskFlag) { return vd_mount(fatFs); }
  else { return pf_mount(fatFs); }
}
...
// The returned value is the resulting status (0 = ok, otherwise see printErrSD())
{
  if(virtDiskFlag) { return vd_open(fileName); }
  else { return pf_open(fileName); }
}
...
//        call readSD() consecutively until EOF is reached
{
  UINT  numBytes;
  byte  errcode;
  if(virtDiskFlag) { errcode = vd_read(buffSD, 32, &numBytes); }
  else { errcode = pf_read(buffSD, 32, &numBytes); }
...
// NOTE3: To finalize the current write operation a writeSD(NULL, &numWrittenBytes) must be called as last action
{
  ...
  if (buffSD != NULL)
  {
    if(virtDiskFlag) { errcode = vd_write(buffSD, 32, &numBytes); }
    else { errcode = pf_write(buffSD, 32, &numBytes); }
  }
  else
  {
    if(virtDiskFlag) { errcode = vd_write(0, 0, &numBytes); }
    else { errcode = pf_write(0, 0, &numBytes); }
  }
...
{
  // byte i;
  if(virtDiskFlag) { return vd_lseek(((unsigned long) sectNum) << 9); }
  else { return pf_lseek(((unsigned long) sectNum) << 9); }
}
...
// ........................................................................
{
  if (errCode)
  {
    if(virtDiskFlag) { Serial.print(F("\r\nIOS: WiFi-VirtDisk error ")); }
    else { Serial.print(F("\r\nIOS: SD error ")); }
  ...
  *(inBuf + 0) = '\0';
  ...
  while( 1 )
  {
    if( Serial.available() > 0 )
    {
      ...
      // Character is not CR or ESC?
      if( ( inChar != 13 ) && ( inChar != 27 ) )
      {
        // Check for Backspace
        if( ( inChar == 8 ) && ( ndx > 0 ) ) { ndx--; }
        else
        { 
        ...
        *(inBuf + ndx) = '\0';      // Terminate string
        ...
        Serial.print( '\r' );
        for( byte i = 0; i <= inCnt; i++ )
        {
          if( *(inBuf + i) != '\0' ) { Serial.print( *(inBuf + i) ); }
          else { break; }
        }
      }
      else
